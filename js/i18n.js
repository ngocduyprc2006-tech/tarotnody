...
(function () {
  ...
  const STRINGS = {
    ...
    'read.opening': { vi: 'Nody giải ngay tại đây nhé: lá này rơi vào vị trí "{label}", mang theo tinh thần của {keys} — nên phần đọc dưới đây không phải để bạn tự đoán, mà là Nody đang nói thẳng nó đang muốn nhắn gì.', en: 'Let Nody walk you through it right here: this card landed on "{label}", carrying the spirit of {keys} — so what follows isn\'t something to guess at, it\'s Nody telling you plainly what it\'s trying to say.', zh: '诺迪现在就直接为你解读：这张牌落在"{label}"的位置，带着{keys}的气息——所以接下来不是让你自己猜，而是诺迪直接告诉你它想说什么。', ko: '노디가 바로 여기서 풀어드릴게요: 이 카드는 "{label}" 자리에 나왔고, {keys}의 기운을 담고 있어요 — 그러니 아래 내용은 스스로 추측할 게 아니라, 노디가 이 카드가 하려는 말을 그대로 짚어드리는 거예요.', ja: 'ノディがここで直接読み解きます：このカードは「{label}」の位置に出て、{keys}の気配をまとっている。だからこの先は推測するものではなく、ノディがこのカードの伝えたいことをそのまま伝えるもの。' },
    'read.why.title': { vi: 'Vì sao Nody đọc ra như vậy', en: 'Why Nody reads it this way', zh: '诺迪为什么这样解读', ko: '노디가 왜 이렇게 해석했는지', ja: 'なぜノディはこう読み解いたのか' },
    'read.why': { vi: 'Các từ khoá {keys} không phải liệt kê cho vui — mỗi từ là một lớp năng lượng riêng của lá bài, và khi cộng ba lớp đó lại với nhau, chúng ghép thành đúng bức tranh mà bạn vừa đọc ở trên. Đó là cách Nody "giải" một lá bài: không đoán mò, mà lần theo từng từ khoá rồi ráp chúng lại.', en: 'The keywords {keys} aren\'t listed just for show — each one is its own layer of the card\'s energy, and stacking those layers together is exactly what forms the picture you just read above. That\'s how Nody actually reads a card: not guesswork, but tracing each keyword and piecing them together.', zh: '{keys}这些关键词不是随便列出来的——每一个都是这张牌的一层能量，把这几层叠加起来，正好构成你刚才读到的那幅画面。这就是诺迪解读一张牌的方式：不是瞎猜，而是顺着每个关键词拼出全貌。', ko: '{keys}라는 키워드들은 그냥 나열해 둔 게 아니에요 — 각각이 이 카드가 지닌 하나의 에너지 층이고, 그 층들을 겹치면 바로 위에서 읽은 그림이 만들어져요. 이게 노디가 카드를 "해석"하는 방식이에요: 대충 추측하는 게 아니라, 키워드 하나하나를 따라가며 맞춰가는 거예요.', ja: '{keys}というキーワードは適当に並べたものではない。それぞれがこのカードの持つエネルギーの層であり、それらを重ね合わせることで、まさに上で読んだあの情景が形作られる。これがノディがカードを「読み解く」やり方——当てずっぽうではなく、キーワードを一つずつたどって組み立てていくということ。' },
    ...
    'synth.opening': { vi: 'Nody ráp {n} lá vừa rút lại thành một mạch cho bạn ngay đây — không phải để bạn tự nối các lá lại, mà Nody đọc thẳng ra bức tranh chung của cả kiểu trải {spread} này trông như thế nào.', en: 'Nody is piecing your {n} cards together into one thread right here — not for you to connect the dots yourself, but Nody laying out plainly what the whole {spread} looks like as a picture.', zh: '诺迪现在就把你抽的{n}张牌拼成一条脉络——不需要你自己去连结，诺迪会直接把整个"{spread}"牌阵的全貌讲给你听。', ko: '노디가 지금 여기서 뽑으신 {n}장의 카드를 하나의 흐름으로 엮어드릴게요 — 스스로 연결할 필요 없이, 노디가 이 {spread} 전체가 어떤 그림인지 그대로 짚어드려요.', ja: 'ノディが今ここで、引いた{n}枚のカードを一つの流れにまとめます——自分でつなぎ合わせる必要はなく、ノディがこの{spread}全体がどんな絵になっているかをそのままお伝えします。' },
